...
test('Hello from console', async ({ page }) => {
    await page.goto('');
    await page.evaluate(() => {
        console.log('Hello from browser'); /* console in browser */
    })
})

test('Get guestData and verify', async ({ page }) => {
    await page.goto('');
    await page.getByText('Guest log in').click();
    await expect(page.locator('//div[@class="panel-page_empty panel-empty"]')).toBeVisible();
    ...
    console.log(guestData); /* console in Node.js */
    let obj = JSON.parse(guestData!);

    expect(obj.expenses).toHaveLength(0);
})

test('Change guestData and verify', async ({ page }) => {
